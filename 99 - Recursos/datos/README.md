...
## Clase 04 · Biomasa de Coiba

La [[2026-09-21 - Clase 04 - Aprendizaje supervisado y Random Forest geoespacial|Clase 04, 21/09/2026]] utiliza la copia local de solo lectura `clase_04/Datos_Coiba.gdb`: puntos con objetivo `AGBD` y 17 rasters. La [procedencia Coiba](clase_04/README.md) identifica origen y límites de uso. La verificación local confirmó los **18 insumos necesarios** (17 rasters y `AGBD_Observaciones_2023_Coiba`, 4.946 puntos): no hace falta copiar datos ni crear otra carpeta de entradas. En el [notebook P01](../notebooks/Clase%2004%20-%20Practica%2001%20-%20Random%20Forest%20geoespacial.ipynb), la sección independiente **«Guía de la práctica en ArcGIS Pro — paso a paso»**, después de la práctica de código, explica conexión local, campos, herramientas y salidas GUI separadas; no cambia los requisitos de ejecución Python.

[P01: Random Forest geoespacial](../notebooks/Clase%2004%20-%20Practica%2001%20-%20Random%20Forest%20geoespacial.ipynb) escribe en una carpeta nueva de `99 - Recursos/salidas_clase_04/practica_01`. La ejecución actual conserva **4.946 puntos: 4.840 completos y 106 fuera del modelo, no eliminados**. La selección se copia a una entidad persistente para gráficos nativos; no es imputación ni limpieza automática. La copia histórica y las evidencias locales originales se conservan por separado; `clase_04` contiene la copia pública preparada para esta práctica.

**Evidencia y límites de P01:** **17/17 celdas exitosas**, incluida PREDICT_RASTER, ArcGIS Pro 3.6.2 / ArcInfo, kernel nuevo, ~320 s; [resultado](../salidas_clase_04/publicados/README.md) y [ejecutado](../notebooks/Clase%2004%20-%20Practica%2001%20-%20Random%20Forest%20geoespacial.ipynb). Esta integración editorial no ejecutó modelos. Mapa raster gris e histograma ArcGIS inspeccionados; 137 enlaces locales sin destinos faltantes en la comprobación independiente previa. Los cinco Mermaid del notebook pasaron parser, renderizado e inspección visual mediante Chrome DevTools MCP y Mermaid 11.12.0; otros diagramas y disposición nativa de Jupyter/ArcGIS Notebook/Obsidian no quedan acreditados. Mapa residual corregido mediante comprobación focal posterior y mapa abierto en Pro verificado, sin reentrenar. AGBD en Mg/ha por confirmación del usuario, no por metadatos. Video parcial, excluido de esta preparación. Validación débil no espacial y extrapolación en los 17 predictores: no es producto certificado ni clase completa. Antecedente de 13/13, [resultados históricos](../salidas_clase_04/publicados/README.md) y dos intentos raster fallidos conservados; cada fallo tuvo 14 éxitos y un error, aunque `completed=15` incluyera el error.

La copia pública conserva **19 datasets** (dos entidades y 17 rasters; 18 entradas activas). Se filtraron metadatos privados preservando créditos y restricciones; atributos, geometrías, CRS y todos los píxeles comparados sin cambios. Coiba y capturas fuente cuentan con autorización expresa de redistribución; la copia histórica privada permanece intacta y no se publica.
